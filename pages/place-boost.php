...
<div class="card">
  ...
  <div class="table-wrapper">
    <table>
      <thead><tr><th>업체</th><th>작업유형</th><th>키워드</th><th>진행률</th><th>상태</th><th>시작일</th><th>제어</th></tr></thead>
...
<script>
const taskTypeNames = {view_boost:'👁️ 조회수 부스팅',keyword_search:'🔍 키워드 검색 유입',review_request:'⭐ 리뷰 유도',photo_update:'📷 사진 업데이트',smart_boost:'🤖 스마트 자동'};
const statusLabels  = {
...
async function loadPlaces() {
  ...
  $('placeList').innerHTML = places.length === 0
    ...
    : places.map(p => {
        ...
        return `<div style="padding:16px;border:1px solid #f0f0f5;border-radius:12px;margin-bottom:10px;">
          <div style="display:flex;align-items:flex-start;justify-content:space-between;">
            ...
            <div style="display:flex;gap:8px;">
              <button class="btn btn-secondary btn-sm" onclick="startBoost(${p.id},'${p.place_name.replace(/'/g,"\\'")}')">⚡ 부스팅</button>
              <button class="btn btn-primary btn-sm" onclick="checkPlaceRank(${p.id},'${p.place_name.replace(/'/g,"\\'")}',${JSON.stringify(kws)})">📊 분석</button>
            </div>
          </div>
...
async function loadTasks() {
  ...
  $('runningTasks').textContent   = running;
  ...
  $('taskTable').innerHTML = tasks.map(t => {
    ...
    return `<tr>
      ...
      <td>
        <div style="display:flex;align-items:center;gap:8px;">
          <div class="progress-wrap" style="width:80px;">
            <div class="progress-fill ${t.status==='running'?'pf-green':t.status==='completed'?'pf-blue':'pf-red'}" style="width:${pct}%"></div>
          </div>
      ...
      <td>
        ${t.status==='running'?`<button class="btn btn-secondary btn-sm" onclick="controlTask(${t.id},'pause')">⏸ 정지</button>`:''}
        ${t.status==='paused'?`<button class="btn btn-success btn-sm" onclick="controlTask(${t.id},'resume')">▶ 재개</button>`:''}
        ${t.status==='completed'||t.status==='failed'?'<span style="color:#aaa;font-size:12px;">-</span>':''}
      </td>
    </tr>`;
  }).join('') || '<tr><td colspan="7" style="text-align:center;color:#888;padding:20px;">작업 내역이 없습니다.</td></tr>';
}
...
async function startBoostTask() {
  ...
  try {
    ...
    const data = await res.json();
    if (data.success) { alert('✅ ' + data.message); $('boostPanel').style.display='none'; loadTasks(); }
    else alert('❌ ' + (data.error||'오류 발생'));
  } finally { hideLoading(); }
}

async function controlTask(id, action) {
  await fetch('index.php?route=api/place-boost/tasks/'+id, {method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({action})});
  loadTasks();
}
...
async function checkRank() {
  ...
  showLoading('순위 확인 중...');
  try {
    const res  = await fetch('index.php?route=api/place-boost/check-rank', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({keyword,place_name:place})});
    const data = await res.json();
    const d    = data.data;
    const el   = $('rankResult');
    ...
    el.innerHTML = `<div style="text-align:center;padding:16px;background:${bg};border-radius:12px;margin-bottom:12px;">
...
async function checkPlaceRank(placeId, placeName, keywords) {
  ...
  for (const kw of keywords.slice(0,3)) {
    const res  = await fetch('index.php?route=api/place-boost/check-rank',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({keyword:kw,place_name:placeName})});
    const data = await res.json();
    if (data.success) results.push({keyword:kw,rank:data.data.rank});
  }
  el.innerHTML = results.map(r=>`<span style="display:inline-flex;align-items:center;gap:4px;background:${r.rank<=3?'#e0f7f0':r.rank<=10?'#fff3e0':'#ffe4e8'};padding:3px 8px;border-radius:6px;font-size:12px;margin-right:6px;">${r.keyword} <strong>${r.rank}위</strong></span>`).join('');
}
...
async function addPlace() {
  ...
  const res  = await fetch('index.php?route=api/place-boost/places',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({place_name:name,category:$('newPlaceCategory').value,address:$('newPlaceAddress').value,naver_place_url:$('newPlaceUrl').value,target_keywords:kws})});
  const data = await res.json();
