...
// 전역 $pdo 세팅 (layout_top / pages / api 에서 global $pdo 로 사용 가능)
...
if (strpos($route, 'api/') === 0) {
    ...
    // ★ 이미 쌓인 출력 버퍼를 모두 비우고 새로 시작
    while (ob_get_level() > 0) ob_end_clean();
    ob_start();

    // ★ 에러가 JSON 앞에 출력되지 않도록 완전히 차단
    ...
    // ★ PHP 치명적 오류도 JSON 으로 잡기
    register_shutdown_function(function() {
        $err = error_get_last();
        if ($err && in_array($err['type'], [E_ERROR, E_PARSE, E_CORE_ERROR, E_COMPILE_ERROR])) {
            // 버퍼에 쌓인 PHP 오류 텍스트 제거
...
function handleApiRoute(string $route): void {
    ...
    if (strpos($route, 'api/settings') === 0) {
        ...
        http_response_code(404);
        echo json_encode(['success' => false, 'error' => 'settings API 없음'], JSON_UNESCAPED_UNICODE);
        return;
    }
